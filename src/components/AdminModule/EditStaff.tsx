...
const EditStaff: React.FC = () => {
    ...
    const fetchStaff = async () => {
        try {
            if (staffId !== undefined) {
                ...
                const response = await apiService.getStaff(Number(userId), parseInt(staffId))
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await apiService.updateStaff(Number(userId), Number(staffId), staff) // userId
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                if (staff.roleId && staff.roleId === docRole && doctor) {
                    try {
                        doctor!.staffId = updatedStaff.staffId
                        // const response = await axios.put(Api.upadteDoctor, doctor);
                        const response = await apiService.upadteDoctor(doctor);
