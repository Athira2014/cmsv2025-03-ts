...
const PatientHistory: React.FC = () => {
    ...
    const fetchAppointments = async () => {
        try {
            setIsLoading(true)
            const response = await apiService.getAppointmentsByPatientId(Number(patientId))
            console.log(response.data.data)
            setAppointments(response.data.data)
            setError(null)
        } catch (error) {
            setError("Error occured while fetching appointments history of a patient:" +patientId)
        }
    }
    ...
    const handleDetailsBtn = (appointment: Appointment) => {
       ...
       if (currentPath.includes('/doctorDash')) {
            navigate(`/doctorDash/appointmentdetails/${userId}/${appointment.appointmentId}`,
                { state: { from: `/doctorDash/patientHistory/${userId}/${patientId}` } }
            )
        } else {
        navigate(`/reception/appointmentdetails/${userId}/${appointment.appointmentId}`,
            { state: { from: `/reception/patientHistory/${userId}/${patientId}` } })
        }
    }
